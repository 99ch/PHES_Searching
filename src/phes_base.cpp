...
Model<short>* read_DEM_with_borders(GridSquare sc, int border) {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        try {
            std::string file1 = file_storage_location + "input/DEMs/" + str(gs) + "_1arc_v3.tif";
            std::string file2 = file_storage_location + "input/DEMs/" + str(gs) + "_FABDEM_V1-2.tif";
            std::string file_to_use;

            bool file1_exists = does_file_exist(file1);
            bool file2_exists = does_file_exist(file2);

            if (file1_exists && file2_exists) {
                throw std::runtime_error("Both DEM files found: " + file1 + " and " + file2);
            ...
            Model<short>* DEM_temp = new Model<short>(file_to_use, GDT_Int16);
            if (i == 0) {
            ...
            search_config.logger.debug("Could not find file " + file_storage_location + "input/DEMs/" + str(gs) + "_1arc_v3.tif or _FABDEM_V1-2.tif " + strerror(errno));
            if (i == 0)
                throw(1);
        }
    }
